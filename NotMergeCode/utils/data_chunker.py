...
class DocumentChunker:
    def __init__(self, chunk_sizes: List[int] = None, chunk_overlap: int = 100):
        """
...
# ---------------------------------------
    def chunk_from_folder(self, data_folder: str, metadata: dict) -> List:
    ...
    def chunk_from_text(self, text_content: str, metadata: dict = None) -> List:
        documents = self.load_documents_from_text(text_content)
        return self.create_hierarchical_chunks(documents, metadata)

    def chunk_from_file(self, file_path: str, metadata: dict = None) -> List:
...
def test_chunker():
    try:
        ...
        sample_text = """
        This is a comprehensive sample document for testing the hierarchical chunking functionality.
        It contains multiple sentences and paragraphs to demonstrate how the hierarchical chunking works with different chunk sizes.
        
        The DocumentChunker class should be able to process this text and create hierarchical nodes at different levels of granularity.
        This means that larger chunks will contain smaller chunks as children, creating a tree-like structure.
        
        Paragraph 1: This is the first paragraph with some detailed content about chunking strategies.
        It explains how documents are broken down into smaller pieces for better processing and retrieval.
        
        Paragraph 2: This is the second paragraph that continues the discussion about hierarchical structures.
        It demonstrates how parent-child relationships work in the context of document chunking.
        
        Paragraph 3: This final paragraph wraps up the discussion and provides a conclusion.
        It shows how all these chunks can be organized in a meaningful hierarchy for better document understanding.
        """
        ...
        print("="*80)
        docx_3_metadata = {
            'Category': ['Customer Value Analysis', 'Customer Segmentation', 'Loyalty & Retention', 'Promotional Tactics', 'Performance Measurement', 'Case Study'],
            'Keywords': ['Decile Analysis', 'Customer Profiling', 'Targeted Promotion', 'Sales Forecasting', 'High-Value Customer Identification']
        }


        # 
        text_nodes = chunker.chunk_from_text(sample_text, docx_3_metadata)
        print(f"Chunked text content: {len(text_nodes)} nodes")

        print(json.dumps(DocumentChunker.to_list_node(text_nodes), indent=2, ensure_ascii=False))
